...
export function AnalysisResult({ analysis, imagePreview }: AnalysisResultProps) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Image Preview */}
            <div className="lg:col-span-1 bg-[#1A1A1A] p-4 rounded-lg border border-[#333333]">
                <img src={imagePreview} alt="Conversation screenshot" className="rounded-md w-full" />
            </div>

            {/* Analysis Report */}
            <div className="lg:col-span-1 bg-[#1A1A1A] p-4 rounded-lg border border-[#333333] space-y-4">
            ...
            <div className="lg:col-span-1 bg-[#1A1A1A] p-4 rounded-lg border border-[#333333] space-y-4">
                ...
                {analysis.suggestions.map((suggestion, index) => (
                    <div key={index} className="bg-[#222222] p-3 rounded-md relative group">
                        <p className="text-sm">{suggestion}</p>
                        <button 
                         onClick={() => handleCopy(suggestion)}
                         className="absolute top-2 right-2 p-1 bg-[#333333] rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
